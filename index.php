...
<html>
    ...
    <body>
        ...
        if (isset($_GET["x"]) && isset($_GET["y"]) && isset($_GET["z"])) {
            ...
            $b = $_GET["y"];
            $c = $_GET["z"];
            $output = json_decode(shell_exec("python3 calculate.py " . escapeshellarg($x) . " " . escapeshellarg($y) . " " . escapeshellarg($z)), true);
            echo "<h2>The calculation result is: $output['sum']</h2>";
        } else {
